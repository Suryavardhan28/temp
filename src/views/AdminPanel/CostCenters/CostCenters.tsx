...
import CostCentersFilter from "./components/CostCentersFilter";
import CostCentersModal from "./components/CostCentersModal";
import {
	type CostCenter,
	type CostCentersTableFilterState,
	type CostCentersTableState,
	constructCostCentersTableFilterObject,
	costCentersTableInitialState,
} from "./utils/costCentersUtils";

const CostCenters: React.FC = () => {
	const { t } = useTranslation("costCenters");
	const [tableState, setTableState] = useState<CostCentersTableState>(
		costCentersTableInitialState,
	);
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [costCenterItem, setCostCenterItem] = useState<
		CostCenter | undefined
	>(undefined);
	const [costCenterMode, setCostCenterMode] = useState<AdminModalMode>(
	...
	const fetchRecords = async () => {
		...
		try {
			const filterObject = constructCostCentersTableFilterObject(
				tableState.filters,
				tableState.page,
			...
			const response = await api.post<
				ApiResponse<PaginatedRecordsApiResponse<CostCenter>>
			>(
				`/api/v1.0/${BACKEND_SERVICES.INVENTORY}/billing-account/cost-center/search?dropdown=false`,
				filterObject,
			);
			...
			dispatch(
				addNotification({
					...
					message: t("errorFetchingCostCentersRecords"),
				}),
			);
		}
	...
	const handleSearch = (filters: CostCentersTableFilterState) => {
		setTableState((prev) => ({
			...prev,
	...
	const handleEdit = (row: CostCenter) => {
		setCostCenterMode(AdminModalMode.EDIT);
		setCostCenterItem(row);
		setIsModalOpen(true);
	};

	const handleCloseModal = () => {
		setCostCenterItem(undefined);
		setIsModalOpen(false);
	};

	const handleAddCostCenter = () => {
		setCostCenterMode(AdminModalMode.CREATE);
		setIsModalOpen(true);
	};
			...
			<Grid container direction="column" sx={{ height: "100%" }} gap={0}>
				...
				>
					...
					<Grid container justifyContent="flex-end" gap={1}>
						...
						<CostCentersFilter onSearch={handleSearch} />
					</Grid>
				</Grid>
				<Grid
					sx={{
