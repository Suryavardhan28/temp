import ArrowDownwardIcon from "@mui/icons-material/ArrowDownward";
import ArrowUpwardIcon from "@mui/icons-material/ArrowUpward";
import EditIcon from "@mui/icons-material/Edit";
import IosShareIcon from "@mui/icons-material/IosShare";
import {
	Box,
	Button,
	Chip,
	Grid2 as Grid,
	IconButton,
	Table,
	TableBody,
	TableCell,
	TableContainer,
	TableHead,
	TableRow,
	Tooltip,
	Typography,
} from "@mui/material";
import { useCallback, useEffect, useState } from "react";
import { useTranslation } from "react-i18next";
import { useDispatch } from "react-redux";
import CustomTableFooterBar from "../../../common/components/CustomTableFooterBar/CustomTableFooterBar";
import CustomizeTableColumns from "../../../common/components/CustomizeTableColumns/CustomizeTableColumns";
import TableBodySkeleton from "../../../common/components/TableSkeleton/TableBodySkeleton";
import { API_STATUS_CODES } from "../../../common/constants/apiStatusCodes";
import { AdminModalMode } from "../../../common/enums/adminModalMode";
import { ItemStatusState } from "../../../common/enums/lineItemStatusStates";
import { NotificationType } from "../../../common/enums/notification";
import { SortOrder } from "../../../common/enums/sortOrder";
import type { ColumnItem } from "../../../common/interfaces/columnItem";
import type { DropDownItem } from "../../../common/interfaces/dropDownItem";
import type { PaginatedRecordsApiResponse } from "../../../common/interfaces/paginatedRecordsApiResponse";
import {
	TableBodyCellStyles,
	TableContainerStyles,
	TableHeaderCellStyles,
	TableStyles,
} from "../../../common/styles/tableStyles";
import { formatDateString } from "../../../common/utils/formatDate";
import { getLighterShade } from "../../../common/utils/getAlphaColor";
import { itemStateColors } from "../../../common/utils/stateColors";
import ContainerLayout from "../../../components/ContainerLayout/ContainerLayout";
import MessageBanner from "../../../components/MessageBanner/MessageBanner";
import useApi, { type ApiResponse } from "../../../config/apiConfig";
import { BACKEND_SERVICES } from "../../../config/appConfig";
import { addNotification } from "../../../redux/slices/Notification/notificationSlice";
import AccountsFilter from "./components/AccountsFilter";
import AccountsModal from "./components/AccountsModal";
import {
	accountsTableInitialState,
	constructAccountsTableFilterObject,
} from "./utils/accountsUtils";
import type {
	Account,
	AccountConfig,
	AccountsTableFilterState,
	AccountsTableState,
} from "./utils/accountsUtils";

const Accounts: React.FC = () => {
	const { t } = useTranslation("accounts");
	const [tableState, setTableState] = useState<AccountsTableState>(
		accountsTableInitialState,
	);
	const [isModalOpen, setIsModalOpen] = useState(false);
	const [accountItem, setAccountItem] = useState<Account | undefined>(
		undefined,
	);
	const [accountMode, setAccountMode] = useState<AdminModalMode>(
		AdminModalMode.CREATE,
	);

	const [accountConfig, setAccountConfig] = useState<AccountConfig[]>([]);

	// Add loading states
	const [isAccountConfigLoading, setIsAccountConfigLoading] = useState(true);
	const api = useApi();
	const dispatch = useDispatch();

	// Fetch Records
	const fetchRecords = async () => {
		setTableState((prev) => ({ ...prev, recordsLoading: true }));
		try {
			const filterObject = constructAccountsTableFilterObject(
				tableState.filters,
				tableState.page,
				tableState.rowsPerPage,
				tableState.sortBy,
				tableState.sortOrder,
			);

			const response = await api.post<
				ApiResponse<PaginatedRecordsApiResponse<Account>>
			>(
				`/api/v1.0/${BACKEND_SERVICES.INVENTORY}/billing-account/accounts/search`,
				filterObject,
			);

			if (response.status_code === API_STATUS_CODES.SUCCESS_OK) {
				const { items, total } = response.data;

				setTableState((prev) => ({
					...prev,
					records: items,
					totalCount: total,
				}));
			} else {
				dispatch(
					addNotification({
						type: NotificationType.ERROR,
						message: t("fetchingAccountsRecords"),
					}),
				);
			}
		} catch (error) {
			dispatch(
				addNotification({
					type: NotificationType.ERROR,
					message: t("fetchingAccountsRecords"),
				}),
			);
		}
		setTableState((prev) => ({ ...prev, recordsLoading: false }));
	};

	useEffect(() => {
		fetchRecords();
	}, [
		tableState.page,
		tableState.rowsPerPage,
		tableState.filters,
		tableState.sortBy,
		tableState.sortOrder,
	]);
	// Fetch and process account data
	useEffect(() => {
		const fetchAccountConfigData = async () => {
			setIsAccountConfigLoading(true);
			try {
				const response = await api.get<ApiResponse<AccountConfig[]>>(
					`/api/v1.0/${BACKEND_SERVICES.INVENTORY}/billing-account/account-types`,
				);
				if (response.status_code === API_STATUS_CODES.SUCCESS_OK) {
					setAccountConfig(response.data);
				} else {
					dispatch(
						addNotification({
							message: t("fetchingAccountConfig"),
							type: NotificationType.ERROR,
						}),
					);
				}
			} catch (error) {
				dispatch(
					addNotification({
						message: t("fetchingAccountConfig"),
						type: NotificationType.ERROR,
					}),
				);
			}
			setIsAccountConfigLoading(false);
		};
		fetchAccountConfigData();
	}, []);

	// Column Visibility
	const handleColumnVisibilityChange = (columns: ColumnItem[]) => {
		const updatedColumns = tableState.columns.map((column) => ({
			...column,
			visible: columns.find((c) => c.id === column.id)?.visible ?? true,
		}));

		setTableState((prev) => ({
			...prev,
			columns: updatedColumns,
		}));
	};

	// Inside InventoryTable component
	const handleSearch = (filters: AccountsTableFilterState) => {
		setTableState((prev) => ({
			...prev,
			filters, // Store all filters in the table state
			page: 1, // Reset to first page when searching
		}));
	};

	const handleSort = useCallback((column: ColumnItem) => {
		setTableState((prev) => {
			if (prev.sortBy?.id === column.id) {
				return {
					...prev,
					sortOrder:
						prev.sortOrder === SortOrder.ASCENDING
							? SortOrder.DESCENDING
							: SortOrder.ASCENDING,
				};
			}
			return {
				...prev,
				sortBy: column,
				sortOrder: SortOrder.ASCENDING,
			};
		});
	}, []);

	// Pagination
	const handleChangePage = (newPage: number) => {
		setTableState((prev) => ({ ...prev, page: newPage }));
	};

	const handleChangeRowsPerPage = (value: number) => {
		setTableState((prev) => ({
			...prev,
			rowsPerPage: value,
			page: 1,
		}));
	};

	const getStatusAttributes = useCallback(
		(isActive: boolean) => {
			const color = isActive
				? itemStateColors[ItemStatusState.Active]
				: itemStateColors[ItemStatusState.Inactive];

			return {
				color: color as string,
				backgroundColor: getLighterShade(color as string),
				text: isActive ? t("active") : t("inactive"),
			};
		},
		[t],
	);

	const handleEdit = (row: Account) => {
		setAccountMode(AdminModalMode.EDIT);
		setAccountItem(row);
		setIsModalOpen(true);
	};

	const handleCloseModal = () => {
		setAccountItem(undefined);
		setIsModalOpen(false);
	};

	const handleAddAccount = () => {
		setAccountMode(AdminModalMode.CREATE);
		setIsModalOpen(true);
	};

	return (
		<ContainerLayout title={t("title")}>
			<Grid container direction="column" sx={{ height: "100%" }} gap={0}>
				<Grid
					container
					justifyContent="space-between"
					sx={{ border: "1px solid transparent" }}
				>
					<CustomizeTableColumns
						columns={tableState.columns}
						onColumnVisibilityChange={handleColumnVisibilityChange}
					/>
					<Grid container justifyContent="flex-end" gap={1}>
						<Button
							disabled={tableState.records.length === 0}
							size="small"
							variant="outlined"
							startIcon={<IosShareIcon fontSize="small" />}
						>
							{t("button.exportToExcel")}
						</Button>
						<AccountsFilter onSearch={handleSearch} />
					</Grid>
				</Grid>
				<Grid
					sx={{
						width: "100%",
						overflow: "hidden",
						pr: "1px",
					}}
				>
					<TableContainer sx={TableContainerStyles}>
						<Table
							stickyHeader
							aria-label="users table"
							sx={TableStyles}
						>
							<TableHead>
								<TableRow>
									<TableCell
										sx={{
											...TableHeaderCellStyles,
											position: "sticky",
											left: 0,
											zIndex: 10,
											width: "50px",
										}}
									>
										<Typography
											variant="body2"
											fontWeight={500}
										>
											{t("actions")}
										</Typography>
									</TableCell>
									{tableState.columns
										.filter(
											(column) =>
												column.visible !== false,
										)
										.map((column) => (
											<TableCell
												key={column.id}
												sx={TableHeaderCellStyles}
												onClick={() =>
													handleSort(column)
												}
											>
												<Grid
													container
													wrap="nowrap"
													alignItems="center"
													justifyContent="space-between"
													gap={1}
												>
													<Typography
														variant="body2"
														fontWeight={500}
													>
														{column.title}
													</Typography>
													{tableState.sortBy?.id ===
														column.id && (
														<Grid
															container
															component="span"
														>
															{tableState.sortOrder ===
															SortOrder.ASCENDING ? (
																<Tooltip
																	title={t(
																		"sortAscending",
																	)}
																>
																	<ArrowUpwardIcon fontSize="small" />
																</Tooltip>
															) : (
																<Tooltip
																	title={t(
																		"sortDescending",
																	)}
																>
																	<ArrowDownwardIcon fontSize="small" />
																</Tooltip>
															)}
														</Grid>
													)}
												</Grid>
											</TableCell>
										))}
								</TableRow>
							</TableHead>
							{tableState.recordsLoading ? (
								<TableBodySkeleton
									readonly={false}
									columnCount={
										tableState.columns.filter(
											(column) =>
												column.visible !== false,
										).length
									}
								/>
							) : (
								<TableBody>
									{tableState.records.length === 0 ? (
										<TableRow>
											<TableCell
												colSpan={
													tableState.columns.filter(
														(col) =>
															col.visible !==
															false,
													).length + 1
												}
												sx={{
													border: "none",
												}}
											>
												<MessageBanner
													type="Table Error"
													title={t("noRecordsFound")}
													description={t(
														"noRecordsFoundDescription",
													)}
													height="40vh"
												/>
											</TableCell>
										</TableRow>
									) : (
										tableState.records.map((row) => {
											return (
												<TableRow key={row.id}>
													<TableCell
														sx={{
															position: "sticky",
															left: 0,
															zIndex: 5,
															...TableBodyCellStyles,
														}}
													>
														<Tooltip
															title={t("edit")}
														>
															<Grid
																container
																justifyContent="center"
															>
																<IconButton
																	sx={{
																		"&:hover":
																			{
																				backgroundColor:
																					"transparent",
																			},
																	}}
																	onClick={() => {
																		handleEdit(
																			row,
																		);
																	}}
																>
																	<EditIcon
																		sx={{
																			color: "primary.A200",
																			backgroundColor:
																				"primary.A50",
																		}}
																	/>
																</IconButton>
															</Grid>
														</Tooltip>
													</TableCell>
													{tableState.columns
														.filter(
															(column) =>
																column.visible !==
																false,
														)
														.map((column) => {
															// Handle different column types
															const renderCell =
																() => {
																	switch (
																		column.id
																	) {
																		case "createdAt":
																		case "updatedAt":
																			return formatDateString(
																				new Date(
																					row[
																						column
																							.id
																					] as string,
																				),
																			);
																		case "isActive": {
																			const status =
																				getStatusAttributes(
																					row.isActive,
																				);
																			return (
																				<Typography
																					variant="body2"
																					color={
																						status.color
																					}
																					bgcolor={
																						status.backgroundColor
																					}
																					component="span"
																					px={
																						1
																					}
																				>
																					{
																						status.text
																					}
																				</Typography>
																			);
																		}
																		case "businessArea":
																		case "product":
																		case "userGroup":
																		case "parentAccountValue":
																		case "serviceProvider":
																		case "accountType":
																		case "accountProductType":
																			return (
																				(
																					row[
																						column.id as keyof Account
																					] as DropDownItem
																				)
																					?.title ||
																				""
																			);
																		case "accountMetadata":
																			// display each key value in new line as a chip
																			return (
																				<Grid
																					container
																					my={
																						1
																					}
																					gap={
																						1
																					}
																					direction="column"
																					wrap="nowrap"
																				>
																					{Object.entries(
																						row.accountMetadata,
																					).map(
																						([
																							key,
																							value,
																						]) => (
																							<Chip
																								color="primary"
																								size="small"
																								variant="outlined"
																								key={
																									key
																								}
																								label={`${key}: ${value}`}
																							/>
																						),
																					)}
																				</Grid>
																			);
																		default:
																			return row[
																				column.id as keyof Account
																			] as string;
																	}
																};

															return (
																<TableCell
																	key={
																		column.id
																	}
																	sx={
																		TableBodyCellStyles
																	}
																>
																	{renderCell()}
																</TableCell>
															);
														})}
												</TableRow>
											);
										})
									)}
								</TableBody>
							)}
						</Table>
					</TableContainer>
					<CustomTableFooterBar
						rowsPerPage={tableState.rowsPerPage}
						totalRows={tableState.totalCount}
						currentPage={tableState.page}
						onRowsPerPageChange={handleChangeRowsPerPage}
						onPageChange={handleChangePage}
					/>
				</Grid>
				<Box flexGrow={1} />
				<Grid container justifyContent="flex-end">
					<Button
						variant="contained"
						color="primary"
						onClick={handleAddAccount}
					>
						{t("button.addAccount")}
					</Button>
				</Grid>
			</Grid>
			<AccountsModal
				mode={accountMode}
				accountItem={accountItem}
				isModalOpen={isModalOpen}
				handleCloseModal={handleCloseModal}
				onSubmit={fetchRecords}
				accountConfig={accountConfig}
				loading={isAccountConfigLoading}
			/>
		</ContainerLayout>
	);
};

export default Accounts;
